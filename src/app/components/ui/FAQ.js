...
export default function FAQ({locale='en'}) {
    ...
    return (
        ...
        >
            <AccordionItem key="1" aria-label="How to download" title={t("How do I download videos?")}>
              {t("Simply paste the video URL into the input box above and click the Download button. Our service will process the video and provide you with a download link.")}
            </AccordionItem>
            <AccordionItem key="2" aria-label="Free service" title={t("Is it really free to use?")}>
              {t("Yes, our service is 100% free to use. There are no hidden charges or registration requirements.")}
            </AccordionItem>
